'''
The below are all the modules that need to be in place for the code to run.
Some are built in to Lambda, and some need importing as custom layers.
Those needed as layers are available in the Layers folder.
More layers will be added in future parts.
'''
#Built in modules:
import json
import ast
...
def lambda_handler(event, context):
    ...
    #Pull out message body in python-readable format
    #Happens to validate that the message is of the expected format (Slack Event)
    try:
